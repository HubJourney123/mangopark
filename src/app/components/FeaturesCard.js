      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={feature.id}
              className={`group relative text-center transform transition-all duration-500 ease-out pt-6 cursor-pointer
            ...
            >
              {/* Number Badge */}
              <div className={`absolute top-0 right-4 w-12 h-12 bg-gradient-to-br from-orange-400 to-red-500 text-white font-bold text-xl rounded-full flex items-center justify-center shadow-lg transform transition-transform duration-300 z-20
              ...
              <div className={`relative bg-gradient-to-br from-orange-50 to-yellow-50 rounded-3xl p-10 md:p-10 pt-12 shadow-lg transition-all duration-500 border border-orange-100 overflow-hidden
                ...
                <div className="relative z-10 mb-6">
                  <div className={`inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-orange-200 to-yellow-200 text-orange-600 rounded-full mb-4 shadow-md transition-all duration-500
                ...
                <div className="relative z-10">
                  <h3 className={`text-xl md:text-2xl font-bold text-gray-800 mb-4 transition-colors duration-300
                ...
                {/* Decorative Elements */}
                <div className={`absolute bottom-4 left-1/2 transform -translate-x-1/2 w-12 h-1 bg-gradient-to-r from-orange-400 to-yellow-400 rounded-full transition-opacity duration-500
                  ${activeCard === feature.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}></div>
                
                {/* Floating Dots */}
                <div className={`absolute top-6 left-6 w-2 h-2 bg-orange-300 rounded-full opacity-50 transition-all duration-500
                  ${activeCard === feature.id ? 'animate-ping' : 'group-hover:animate-ping'}`}></div>
                <div className={`absolute bottom-8 right-8 w-3 h-3 bg-yellow-300 rounded-full opacity-50 transition-all duration-500
                  ${activeCard === feature.id ? 'animate-pulse' : 'group-hover:animate-pulse'}`}></div>
              </div>

              {/* Hover Glow Effect */}
              <div className={`absolute inset-0 bg-gradient-to-r from-orange-200 to-yellow-200 rounded-3xl blur-xl transition-opacity duration-500 -z-10
